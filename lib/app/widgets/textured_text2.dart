...
class TexturedText2 extends StatelessWidget {
  const TexturedText2({Key? key}) : super(key: key);
...
class NegativeMaskedImageDemo extends StatelessWidget {
  const NegativeMaskedImageDemo({Key? key, this.text, this.image})
      : super(key: key);
  final text;
  final image;
        ...
        child: Container(
          child: ImageFilledText(
            text: ScaledLayeredText(
              textScale: 1,
              child: const Text(
                'WIDGET MASK which is always work',
                style: TextStyle(
            ...
            image: const AssetImage('assets/images/texture1.png'),
          ),
        ),
      );
}
